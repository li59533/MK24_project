...
{   
	MB_WirteRegister(MBREGISTERINPUT, 17,VERSION_MAJOR);
	MB_WirteRegister(MBREGISTERINPUT, 18,VERSION_MINOR);
	MB_WirteRegister(MBREGISTERINPUT, 19,VERSION_LITE);
...
void MBFunction_03(void)
{
    ...
    if ((addr >= Slave.StartAddr) && (addr + ask_data_count <= Slave.EndAddr+1))
    {
